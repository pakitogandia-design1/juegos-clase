...
function nav(n){
  try {
    ...
    (routes[n] || routes.main)();
  } catch (e) {
    console.error('Error navegando a', n, e);
    toast('He reparado un menú. Prueba de nuevo.');
  }
}
...
function defaultForType(type){
  const preferred = ECONOMY.defaultUnlocked?.[type];
  if(preferred && COLLECTIBLES.some(c=>c.id===preferred && c.type===type)) return preferred;
  return COLLECTIBLES.find(c=>c.type===type && c.unlockDefault)?.id || COLLECTIBLES.find(c=>c.type===type)?.id;
}
function badgeProfileHTML(){
  const badge = equippedItem('badge');
  const rarity = badge?.rarity || 'common';
  const name = badge?.name || 'Primer Orbe';
  return `<div class="profile-card rarity-${rarity}">
    <div class="profile-frame"><div class="profile-badge">${badge?iconFor(badge):'🎖️'}</div></div>
    <div>
      <b>Perfil Snakademy</b>
      <span>${name}</span>
      <small>Insignia equipada · marco ${RARITY[rarity] || 'Común'}</small>
    </div>
  </div>`;
}
...
function snakePreviewHTML(){
  ...
  }).join('')}</div>${petHTML()}${badgeProfileHTML()}`;
}
function showMain(){
...
function drawTrail(ctx, snake){
  ...
  body.forEach((s,i)=>{
    if(i%2 && body.length>7) return;
    const px=BOARD.x+s.x*BOARD.cell+16, py=BOARD.y+s.y*BOARD.cell+16;
    const offset = (i%4);
    const ox = [-15, 15, -12, 12][offset];
    const oy = [17, 17, -12, -12][offset];
    ctx.save();
    ctx.globalAlpha=Math.max(.18,.56-i*.025);
    ctx.fillStyle=tr.color;
    ctx.shadowColor=tr.color;
    ctx.shadowBlur=10;
    ctx.font='900 22px system-ui';
    ctx.fillText(marks[tr.kind]||'✦',px+ox,py+oy);
    ctx.restore();
  });
}
...
function showCollection(tab='skin', selected=null){
  ...
  const cards=list.map(i=>{
    const un=tab==='achievements'?state.achievements.includes(i.id):unlocked(i.id);
    const nw=state.newCollectibles.includes(i.id);
    const eq=Object.values(state.equipped).includes(i.id);
    return `<div class="card collect rarity-${i.rarity} ${un?'':'locked'} ${eq?'equipped-card':''}" data-item="${i.id}">
      <div class="icon">${un?iconFor(i):'❔'}</div>
      <b>${un?i.name:'Aún no descubierto'}</b>
      <p>${tab==='achievements' ? (un?'Logro conseguido':'Logro pendiente') : (eq?'Clic para desequipar':un?'Clic para equipar':RARITY[i.rarity]||'')}</p>
      ${nw?'<span class="new">Nuevo</span>':''}${eq?'<span class="new" style="left:8px;right:auto">Equipado</span>':''}
    </div>`;
  }).join('');
  const un=sel && (tab==='achievements'?state.achievements.includes(sel.id):unlocked(sel.id));
  const equipped = sel && Object.values(state.equipped).includes(sel.id);
  const preview = sel?.type==='badge' && un ? `<div class="profile-card rarity-${sel.rarity}" style="position:static;margin:0 auto 16px;max-width:270px"><div class="profile-frame"><div class="profile-badge">${iconFor(sel)}</div></div><div><b>${sel.name}</b><span>${equipped?'Equipada':'Insignia'}</span><small>Marco ${RARITY[sel.rarity]||''}</small></div></div>` : `<div style="font-size:100px;text-align:center">${un?iconFor(sel):'❔'}</div>`;
  setScreen(`${top('Colección','main')}<section class="full"><div class="tabs">${tabs}</div><div class="collection"><div class="collection-grid">${cards}</div><div class="panel rarity-${sel?.rarity||'common'}">${preview}<h2>${un?sel.name:'Objeto misterioso'}</h2><p>${un?sel.description:'Abre cajas o completa logros para descubrirlo.'}</p>${tab!=='achievements'&&un?`<p class="sub">${equipped?'Ya está equipado. Haz clic en su tarjeta para desequiparlo.':'Haz clic en su tarjeta para equiparlo automáticamente.'}</p>`:''}</div></div></section>`);
  $all('[data-tab]').forEach(b=>b.onclick=()=>showCollection(b.dataset.tab));
  $all('[data-item]').forEach(b=>b.onclick=()=>{
    const clicked = list.find(i=>i.id===b.dataset.item);
    const canToggle = tab!=='achievements' && clicked && unlocked(clicked.id);
    if(canToggle){ toggleEquip(clicked.id); showCollection(tab,clicked.id); }
    else showCollection(tab,b.dataset.item);
  });
}
function equip(id){
  const it=item(id);
  if(!it||!unlocked(id)) return toast('Bloqueado');
  state.equipped[it.type]=id;
  state.newCollectibles=state.newCollectibles.filter(x=>x!==id);
  save(); snd('click'); toast('Equipado');
}
function toggleEquip(id){
  const it=item(id);
  if(!it||!unlocked(id)) return toast('Bloqueado');
  const def = defaultForType(it.type);
  if(state.equipped[it.type]===id){
    state.equipped[it.type]=def || id;
    state.newCollectibles=state.newCollectibles.filter(x=>x!==id);
    save(); snd('click');
    toast(id===def ? 'Equipamiento inicial activo' : 'Desequipado');
    return;
  }
  state.equipped[it.type]=id;
  state.newCollectibles=state.newCollectibles.filter(x=>x!==id);
...
function showGuide(){
  setScreen(`${top('Guía','main')}<section class="full scroll"><div class="panel"><h1 class="title">Cómo se juega</h1><p class="sub">Snakademy mezcla Snake clásico con mutaciones roguelike.</p><div class="grid cards">
    ${[
      ['🎮','Controles','PC: WASD/Flechas. Móvil: toca lado izquierdo/derecho del tablero para girar.'],
      ['🧪','Expedición','20 cámaras, mutaciones entre cámaras y jefes.'],
      ['♾️','Infinito','No cambia de pantalla. Cada 10 orbes sube el nivel.'],
      ['🟢','Orbes','Verde comida, dorado monedas, azul Fase, rojo riesgo, morado mutación, negro trampa.'],
      ['⚡','Fase Snake','Atraviesa tu cola durante unos segundos.'],
