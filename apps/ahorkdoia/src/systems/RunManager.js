...
export class RunManager{
  ...
  create({categoryKey='jardineria',difficulty='normal',daily=false}={}){
    ...
    const run={id:seed,seed,categoryKey,difficulty,daily,total,nodeIndex:0,nodes,act:1,datos:0,saved:0,lost:0,powerups:[],lettersUsed:{},completed:false,failed:false,bosses:[]};
  ...
  completeNode(run){run.nodes[run.nodeIndex].completed=true; run.nodeIndex++; if(run.nodeIndex>=run.total){run.completed=true; this.save.stats.runsCompleted++; if(run.daily){this.save.stats.dailyCompleted++; this.save.lastDaily={date:todayKey(),completed:true,result:{saved:run.saved,lost:run.lost}};} } }
}
