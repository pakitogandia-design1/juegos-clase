...
export class AchievementManager{
  ...
  evaluate(){
    ...
    if((eco.civiliansBought||0)>=1)u('first_purchase'); if((eco.civiliansBought||0)>=5)u('buy_5'); if((eco.civiliansBought||0)>=10)u('buy_10');
    [10,25,40,60].forEach(n=>{if(civs.length>=n)u('civ_'+n)}); [25,75,150,300,500].forEach(n=>{if(codex.length>=n)u('codex_'+n)});
    if(civs.some(c=>c.rarity==='legendario'))u('legend_civ');
    const byCat={}; codex.forEach(c=>byCat[c.categoryKey]=(byCat[c.categoryKey]||0)+1); Object.entries(byCat).forEach(([k,v])=>{if(v>=25)u('cat_'+k+'_25')});
