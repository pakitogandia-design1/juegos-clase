...
export class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    this.selected={x:4,y:4}; this.activeSlot=this.state.activeSlot||0; this.currentPlant=this.state.selectedPlantIndex||0; this.isPaused=false;
    this.worldLayer=this.add.container(0,0); this.fxLayer=this.add.container(0,0); this.drawMap(); this.createPlayer(); this.setupCamera(); this.createInput();
  ...
  pointerToIso(pointer){
    const worldPoint=this.cameras.main.getWorldPoint(pointer.x,pointer.y);
    return this.screenToIso(worldPoint.x,worldPoint.y);
  }
  setupCamera(){
    const points=[];
    for(let y=0;y<this.gridH;y++) for(let x=0;x<this.gridW;x++){ const p=this.isoToScreen(x,y); points.push(p); }
    const xs=points.map(p=>p.sx), ys=points.map(p=>p.sy);
    const minX=Math.min(...xs)-this.tileW*2; const maxX=Math.max(...xs)+this.tileW*2;
    const minY=Math.min(...ys)-this.tileH*2; const maxY=Math.max(...ys)+this.tileH*4;
  ...
  setCurrentPlant(index){
    ...
    this.state.selectedPlantId=PLANTS[this.currentPlant]?.id;
    this.ui()?.refreshHud();
    this.saveSoon();
  }
  createInput(){
    ...
    this.input.on('pointermove',p=>{ if(this.isPaused) return; const t=this.pointerToIso(p); if(this.valid(t.x,t.y)){ this.selected=t; this.updateSelector(); this.ui()?.refreshHud(); }});
    this.input.on('pointerdown',p=>{ if(this.isPaused) return; this.soundSys.ensure(); const t=this.pointerToIso(p); if(this.valid(t.x,t.y)){ this.selected=t; this.updateSelector(); this.act(); }});
    ...
    ['ONE','TWO','THREE','FOUR','FIVE','SIX','SEVEN','EIGHT','NINE'].forEach((k,i)=>this.input.keyboard.on('keydown-'+k,()=>{this.setActiveSlot(i); this.soundSys.beep('click');}));
    this.input.keyboard.on('keydown-Q',()=>this.cyclePlant(-1)); this.input.keyboard.on('keydown-R',()=>this.cyclePlant(1));
  }
  ...
  toolbar(){ if(!this.state.quickSlots) this.state.quickSlots=['hoe','shovel','watering','pruners','sprayer','seed','compost_item','path','hand']; return this.state.quickSlots; }
  setActiveSlot(i){ this.activeSlot=Phaser.Math.Clamp(i,0,this.toolbar().length-1); this.state.activeSlot=this.activeSlot; this.ui()?.refreshHud(); this.saveSoon(); }
  setQuickSlot(i,itemId){ const slots=this.toolbar(); slots[i]=itemId; this.state.quickSlots=slots; this.activeSlot=i; this.state.activeSlot=i; this.ui()?.refreshHud(); this.floatText(`${this.displayName(itemId)} enviado a barra rápida.`,true); this.saveSoon(); }
  cyclePlant(dir){ this.setCurrentPlant(this.currentPlant+dir); this.floatText('Planta preparada: '+this.getActivePlant().common,true); }
  displayName(id){ return PLANTS.find(p=>p.id===id)?.common || [...TOOLS,...BLOCKS,...RESOURCES].find(x=>x.id===id)?.name || DECORATIONS.find(d=>d.id===id)?.name || id; }
  ...
  act(){ const tile=this.map[this.selected.y][this.selected.x]; const item=this.activeItem(); let res={ok:false,msg:'Nada que hacer.'};
    ...
    else if(item==='seed'){ const plant=PLANTS[this.currentPlant%PLANTS.length]; res=this.plants.plantAt(tile,plant.id); if(res.ok && this.state.mode!=='creative'){ if(this.inv.has(plant.id,1)) this.inv.remove(plant.id,1); else this.inv.remove('seed',1); } }
    else if(BLOCKS.some(b=>b.id===item)){ tile.block=item; this.state.selectedBlock=item; this.ach.add('green_architect',1); if(this.state.mode!=='creative') this.inv.remove(item,1); res={ok:true,msg:this.displayName(item)+' colocado.'}; }
    else if(DECORATIONS.some(d=>d.id===item)){ tile.decor=item; this.ach.add('green_architect',1); if(this.state.mode!=='creative') this.inv.remove(item,1); res={ok:true,msg:this.displayName(item)+' colocado.'}; }
  ...
  floatText(msg,ok=true){ const t=this.add.text(640,588,msg,{fontFamily:'monospace',fontSize:18,color:ok?'#dfffc0':'#ffd0b0',backgroundColor:'#101820ee',padding:{x:16,y:8}}).setOrigin(.5).setScrollFactor(0).setDepth(20000); this.tweens.add({targets:t,y:560,alpha:0,duration:2100,onComplete:()=>t.destroy()}); }
  ...
  getSuggestion(){
    ...
    if(!tile.plant && active!=='seed') return `Siguiente paso: elige una planta en Inventario o usa Q/R para cambiar. Para sembrar ${plant.common}, selecciona Semillas en la barra rápida.`;
    if(!tile.plant && active==='seed') return `Acción actual: plantar ${plant.common}. Busca tierra labrada o suelo fértil y pulsa acción.`;
    if(tile.plant){
  ...
  save(){ this.state.map=this.map; this.state.activeSlot=this.activeSlot; this.state.selectedPlantIndex=this.currentPlant; this.state.selectedPlantId=this.getActivePlant()?.id; SaveSystem.save(this.state); }
}
