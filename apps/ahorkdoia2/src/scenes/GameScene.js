...
export class GameScene extends Phaser.Scene{
 ...
 create(){
  ...
  this.used=new Set(); this.revealed=new Set(); this.disabledLetters=new Set(); this.hintLevel=this.run.difficulty==='hard'?0:1; if(this.run.powerups?.some(p=>p.type==='extra_start_hint')) this.hintLevel=Math.min(2,this.hintLevel+1);
 ...
 draw(){
  bg(this); title(this,this.boss?`BOSS: ${this.boss.name}`:'Rescate Civilian',`${this.word.categoria} · ${this.word.subcategoria}`); const w=this.scale.width,h=this.scale.height;
  ...
  button(this,105,h-38,160,42,'Pista +1 fallo',()=>this.buyHint(),{size:'14px',stroke:0xffe14a});
 ...
 currentHintText(){ const k='pista'+this.hintLevel; return `PISTA ${this.hintLevel}: ${this.word[k]||this.word.pista4}`;}
 ...
 buyHint(){ if(this.hintLevel>=4){this.status.setText('K-IA no tiene más datos que filtrar.'); return;} let free=false; if(this.freeEmergency){free=true; this.freeEmergency=false;} if(this.run.freeHints){free=true; this.run.freeHints--;} if(this.powerups.consumeType(this.run,'free_hint')) free=true; if(!free){this.hintsBought++; this.save.stats.hintsBought++; this.applyFail('La pista tiene precio. +1 fallo.'); if(this.fails>=this.maxFails) return;} this.hintLevel++; this.msg.setText(this.currentHintText()); this.status.setText(free?'Pista gratis filtrada por K-IA.':'Pista comprada. Información recuperada.'); this.sounder.play('loot'); this.refreshPowerupsPanel();}
 usePowerup(id){
  ...
  else if(type==='reveal_category'){ this.status.setText(`Categoría revelada: ${this.word.categoria} · ${this.word.subcategoria}`); ok=true; }
