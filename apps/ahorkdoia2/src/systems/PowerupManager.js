...
export class PowerupManager{
  ...
  count(run,type){return (run.powerups||[]).filter(p=>p.type===type).length}
  consumeType(run,type){ const i=(run.powerups||[]).findIndex(p=>p.type===type); if(i>=0){return run.powerups.splice(i,1)[0];} return null;}
  consumeId(run,id){ const i=(run.powerups||[]).findIndex(p=>p.id===id); if(i>=0){return run.powerups.splice(i,1)[0];} return null;}
  isManual(p){ return ['reveal_vowel','reveal_consonant','free_hint','heal','remove_wrong','first_letter','last_letter','strong_reveal','random_good','reveal_category','double_hint_curse'].includes(p.type); }
  isPassive(p){ return !this.isManual(p); }
  manualPowerups(run){ return (run.powerups||[]).filter(p=>this.isManual(p)); }
}
