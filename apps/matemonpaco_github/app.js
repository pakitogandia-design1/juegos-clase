...
function renderSetupLabels() {
  ...
  $('#teamHelp').textContent = ai
    ? 'Elige 3 MateMon para tu equipo. Paco formará automáticamente un equipo rival pensado para contrarrestar tus elecciones.'
...
function renderMiniTeam(elementId, team) {
  ...
  const isAutoAiTeam = state.gameMode === 'ai' && elementId === 'teamB';
  wrap.innerHTML = '';
  for (let index = 0; index < 3; index++) {
    const card = getCard(team[index]);
    const slot = document.createElement('div');
    slot.className = 'mini-slot' + (!card ? ' empty' : '') + (isAutoAiTeam ? ' auto-ai-slot' : '');
    slot.innerHTML = card
      ? `<img src="${card.image}" alt="${card.name}"><strong>${card.name}</strong>${isAutoAiTeam ? '<small>Elección de Paco</small>' : '<button title="Quitar carta">×</button>'}`
      : (isAutoAiTeam ? `Paco elegirá rival ${index + 1}` : `Hueco ${index + 1}`);
    if (card && !isAutoAiTeam) {
      slot.querySelector('button').addEventListener('click', () => {
        team.splice(index, 1);
        if (state.gameMode === 'ai') buildSmartAiTeam();
        renderAll();
      });
    }
  ...
  cards.forEach((card) => {
    ...
    button.innerHTML = `
      ...
      <div class="card-content">
        ...
        ${state.gameMode === 'ai' && selectedB ? `<p class="ai-reason">${explainAiPick(card, state.teamA)}</p>` : ''}
...
function bestMoveAnalysis(attacker, defender) {
  const options = attacker.moves.map((move, index) => {
    const calc = calculateDamage(attacker, defender, move);
    return {
      index,
      move,
...
function scoreAiCandidate(candidate, playerTeam) {
  if (!playerTeam.length) return 0;
  let score = 0;
  playerTeam.forEach((playerId) => {
    const enemy = getCard(playerId);
    const outgoing = bestMoveAnalysis(candidate, enemy);
    const incoming = bestMoveAnalysis(enemy, candidate);
    const offenseRatio = outgoing.damage / Math.max(1, Number(enemy.hp));
    const defenseRatio = incoming.damage / Math.max(1, Number(candidate.hp));
    score += offenseRatio * 100;
    score += outgoing.multiplier * 10;
    score += outgoing.priority ? 5 : 0;
    score += Number(candidate.speed) > Number(enemy.speed) ? 4 : 0;
    score -= defenseRatio * 65;
  });
  score += Number(candidate.hp) * 0.08;
  score += Number(candidate.speed) * 0.05;
  return score;
}

function explainAiPick(card, playerTeam) {
  if (!playerTeam.length) return 'Paco espera tus elecciones.';
  const bestTarget = playerTeam
    .map((id) => {
      const target = getCard(id);
      const attack = bestMoveAnalysis(card, target);
      const threat = bestMoveAnalysis(target, card);
      return {
        target,
        attack,
        value: (attack.damage / Math.max(1, Number(target.hp))) - (threat.damage / Math.max(1, Number(card.hp)))
      };
    })
    .sort((a, b) => b.value - a.value)[0];
  return `Paco lo elige porque ${bestTarget.attack.move.name} hace ${bestTarget.attack.damage} PS a ${bestTarget.target.name}.`;
}

function buildSmartAiTeam() {
  if (state.gameMode !== 'ai') return;
  state.teamB = [];
  const playerTeam = state.teamA.slice();
  if (!playerTeam.length) return;

  const used = new Set(playerTeam);
  const available = state.cards
    .filter((card) => !used.has(card.id))
    .map((card) => ({ card, score: scoreAiCandidate(card, playerTeam) }))
    .sort((a, b) => b.score - a.score || a.card.name.localeCompare(b.card.name, 'es'));

  for (const playerId of playerTeam) {
    if (state.teamB.length >= 3) break;
    const target = getCard(playerId);
    const bestCounter = available
      .filter((entry) => !state.teamB.includes(entry.card.id))
      .map((entry) => {
        const outgoing = bestMoveAnalysis(entry.card, target);
        const incoming = bestMoveAnalysis(target, entry.card);
        const counterScore = (outgoing.damage / Math.max(1, Number(target.hp))) * 120
          + outgoing.multiplier * 12
          + (outgoing.priority ? 5 : 0)
          + (Number(entry.card.speed) > Number(target.speed) ? 4 : 0)
          - (incoming.damage / Math.max(1, Number(entry.card.hp))) * 55;
        return { ...entry, counterScore };
      })
      .sort((a, b) => b.counterScore - a.counterScore || b.score - a.score)[0];
    if (bestCounter) state.teamB.push(bestCounter.card.id);
  }

  for (const entry of available) {
    if (state.teamB.length >= 3) break;
    if (!state.teamB.includes(entry.card.id)) state.teamB.push(entry.card.id);
  }
}
