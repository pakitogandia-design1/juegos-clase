...
export class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    this.selected={x:4,y:4}; this.activeSlot=0; this.currentPlant=0; this.isPaused=false;
    ...
    if(!this.state.tutorialDone && this.state.mode==='adventure') this.time.delayedCall(500,()=>this.ui()?.openPanel('tutorial'));
  ...
  createPlayer(){ this.player={x:4,y:4,dir:'down'}; const p=this.isoToScreen(4,4); this.playerSprite=this.add.image(p.sx,p.sy+8,'player_down').setOrigin(.5,.85).setDepth(10000); }
  createInput(){
  ...
  handleMove(delta){ const k=this.keys; if(this.moveLock && this.time.now<this.moveLock) return; let dx=0,dy=0,dir=this.player.dir; if(k.W.isDown||k.UP.isDown){dy=-1;dir='up';} else if(k.S.isDown||k.DOWN.isDown){dy=1;dir='down';} else if(k.A.isDown||k.LEFT.isDown){dx=-1;dir='left';} else if(k.D.isDown||k.RIGHT.isDown){dx=1;dir='right';}
    if(dx||dy){ const nx=this.player.x+dx, ny=this.player.y+dy; if(this.valid(nx,ny)){ this.player.x=nx; this.player.y=ny; this.selected={x:nx,y:ny}; const p=this.isoToScreen(nx,ny); this.tweens.add({targets:this.playerSprite,x:p.sx,y:p.sy+8,duration:110,ease:'Sine.easeOut'}); } this.player.dir=dir; this.playerSprite.setTexture('player_'+dir); this.moveLock=this.time.now+130; this.updateSelector(); this.ui()?.refreshHud(); }}
