...
export default class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    this.physics.world.on('worldbounds', (body) => {
      ...
      if (body.blocked.down || body.blocked.up) {
        const minUp = b.sizeLevel >= 4 ? 470 : (b.sizeLevel === 3 ? 410 : 350);
        if (Math.abs(b.body.velocity.y) < minUp * .55) b.setVelocityY(-minUp);
      }
      if (Math.abs(b.body.velocity.x) < 80) {
        const dir = b.x < 480 ? 1 : -1;
        b.setVelocityX(dir * 130 * this.mode.bubbleSpeed);
      }
    });
    this.harpoons = new HarpoonSystem(this);
    this.powerups = new PowerUpSystem(this, this.mode.powerRate);

    this.physics.add.collider(this.bubbles.group, this.platformSystem.platforms, (bubble) => {
      if (!bubble || !bubble.body) return;
      if (Math.abs(bubble.body.velocity.x) < 80) {
        bubble.setVelocityX((bubble.x < 480 ? 1 : -1) * 125 * this.mode.bubbleSpeed);
      }
      if (bubble.body.blocked.down || bubble.body.touching.down) {
        const bounceY = bubble.sizeLevel >= 4 ? 455 : (bubble.sizeLevel === 3 ? 395 : 335);
        bubble.setVelocityY(-bounceY);
      }
    });
    ...
    this.keys = this.input.keyboard.addKeys({
      ...
      Z:Phaser.Input.Keyboard.KeyCodes.Z,X:Phaser.Input.Keyboard.KeyCodes.X,E:Phaser.Input.Keyboard.KeyCodes.E,
      P:Phaser.Input.Keyboard.KeyCodes.P, ESC:Phaser.Input.Keyboard.KeyCodes.ESC
    });
    this.touch = {left:false,right:false,up:false,down:false,shoot:false,shootRational:false,shootIrrational:false,weapon:false};
  ...
  createHud(){
    ...
    this.harpoonLabel = this.add.text(800,18,'',{fontSize:14,color:'#fff',fontStyle:'bold'}).setOrigin(.5);
    this.weaponIcon = this.add.text(930,18,'⤊',{fontSize:22,color:'#fde68a',fontStyle:'bold'}).setOrigin(.5);
    this.hud.add([this.hudBg,this.hudText,this.harpoonLabel,this.weaponIcon]);
  }
  ...
  updateHud(){
    ...
    const weaponName = this.currentWeapon === 'harpoon' ? 'Arpón' : 'Escopeta';
    const typeName = this.currentHarpoon === 'rational' ? 'RACIONAL' : 'IRRACIONAL';
    this.harpoonLabel.setText(`${weaponName} ${typeName} · Z=R · X=I · E=arma`);
    this.harpoonLabel.setColor(this.currentHarpoon === 'rational' ? '#7dd3fc' : '#f0abfc');
    if (this.weaponIcon) {
      this.weaponIcon.setText(this.currentWeapon === 'harpoon' ? '⤊' : '⇢');
      this.weaponIcon.setColor(this.currentWeapon === 'harpoon' ? '#93c5fd' : '#fde68a');
    }
  }
  ...
  createTouchControls(){
    ...
    make(704,468,'R','shootRational'); make(776,468,'I','shootIrrational'); make(848,468,'⇄','change'); make(920,468,'⚙','weapon');

    this.add.text(704,506,'racional',{fontSize:10,color:'#bae6fd',fontStyle:'bold'}).setOrigin(.5).setDepth(81);
    this.add.text(776,506,'irracional',{fontSize:10,color:'#f5d0fe',fontStyle:'bold'}).setOrigin(.5).setDepth(81);
    this.add.text(920,506,'arma',{fontSize:10,color:'#fde68a',fontStyle:'bold'}).setOrigin(.5).setDepth(81);
  }
  ...
  switchWeapon(){
    this.currentWeapon = this.currentWeapon === 'harpoon' ? 'shotgun' : 'harpoon';
    const label = this.currentWeapon === 'harpoon' ? 'ARMA: ARPÓN VERTICAL' : 'ARMA: ESCOPETA HORIZONTAL';
    this.floatText(this.player.sprite.x, this.player.sprite.y-72, label, this.currentWeapon === 'harpoon' ? '#93c5fd' : '#fde68a');
    this.updateHud();
  }

  fireSelected(type = this.currentHarpoon) {
    this.currentHarpoon = type;
    this.updateHud();
    if (this.currentWeapon === 'shotgun') {
      return this.harpoons.shootShotgun(this.player.sprite.x, this.player.sprite.y, type, this.player.facing || 1);
    }
    return this.harpoons.shoot(this.player.sprite.x, this.player.sprite.y-28, type);
  }


  update(time, delta){
    if (this.bubbles) this.bubbles.syncLabels();
    ...
    if (Phaser.Input.Keyboard.JustDown(this.keys.E) || this.touch.weapon) {
      this.touch.weapon = false;
      this.switchWeapon();
    }
    ...
    if (Phaser.Input.Keyboard.JustDown(this.keys.Z) || this.touch.shootRational) {
      this.touch.shootRational = false;
      this.fireSelected('rational');
    }
    if (Phaser.Input.Keyboard.JustDown(this.keys.X) || this.touch.shootIrrational) {
      this.touch.shootIrrational = false;
      this.fireSelected('irrational');
    }
    if (Phaser.Input.Keyboard.JustDown(this.keys.SPACE) || this.touch.shoot) {
      this.touch.shoot = false;
      this.fireSelected(this.currentHarpoon);
    }
    if (this.mode.timer) {
  ...
  hitBubble(harpoon, bubble){
    ...
    const correct = harpoon.attackType === bubble.category;
    ...
    if (correct) {
      ...
      this.bubbles.destroyBubble(bubble);
