...
export default class MenuScene extends Phaser.Scene {
  ...
  draw(){
    ...
    const buttons = [
      ...
      {label:'Autoaventura', action:()=>{localStorage.removeItem(SAVE_KEY); this.scene.start('GameScene',{newGame:true, autoAdventure:true});}},
      {label:'Instrucciones', action:()=>this.showInstructions()},
      {label:'Créditos', action:()=>this.scene.start('CreditsScene')}
    ];
    const startY = h*.44;
    buttons.forEach((b,i)=>this.button(w/2, startY+i*58, Math.min(420,w*.78), 46, b.label, b.action));
