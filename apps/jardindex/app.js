...
if(!state.data.dex){
  state.data = {dex:{}, garden:{}, foundCount:0, completedCount:0, sound:true, difficulty:"Técnico", player:{name:"", group:""}};
}
function normalizeSaveData(){
  state.data.dex = state.data.dex || {};
  state.data.garden = state.data.garden || {};
  state.data.sound = state.data.sound !== false;
  state.data.difficulty = state.data.difficulty || "Técnico";
  state.data.player = state.data.player || {name:"", group:""};
  state.data.player.name = state.data.player.name || "";
  state.data.player.group = state.data.player.group || "";
  state.data.updatedAt = state.data.updatedAt || new Date().toISOString();
}
normalizeSaveData();
const app = document.getElementById("app");
const $ = (sel,root=document)=>root.querySelector(sel);
const $$ = (sel,root=document)=>Array.from(root.querySelectorAll(sel));
function save(){state.data.updatedAt=new Date().toISOString();localStorage.setItem("jardindex_fp_safor_save",JSON.stringify(state.data));}
...
function header(){
  ...
  return `<div class="topbar">
    ...
    <div class="nav">
      <button onclick="go('home')">Inicio</button><button onclick="go('explore')">Explorar</button><button onclick="go('dex')">JardiDex</button><button onclick="go('zones')">Zonas</button><button onclick="go('garden')">Mantener</button><button onclick="go('profile')">Perfil</button><button onclick="go('credits')">Créditos</button>
    </div>
  </div>
  <div class="stats-row">
    <div class="stat"><b>${found}/74</b> vistas</div><div class="stat"><b>${implanted}</b> en mantenimiento</div><div class="stat"><b>${complete}</b> doradas</div><div class="stat"><b>${state.data.difficulty}</b> dificultad</div><div class="stat"><b>${state.data.player.name?escapeHtml(state.data.player.name):"Sin nombre"}</b> perfil</div>
  </div>`;
}
    ...
    <section class="card dark hero">
      <div>
        ...
        <div class="stats-row">
          ...
          <button class="btn" onclick="go('profile')">👤 Perfil y código</button>
...
function niceDate(iso){try{return iso?new Date(iso).toLocaleString('es-ES'):"sin registro";}catch(e){return "sin registro";}}
...
function profileStats(){
  const dex=Object.values(state.data.dex||{}), garden=Object.values(state.data.garden||{});
  return {
    seen:dex.filter(x=>x.status&&x.status!=='unknown').length,
    implanted:garden.length,
    gold:dex.filter(x=>x.status==='gold').length,
    dead:garden.filter(x=>x.dead).length,
    zoned:dex.filter(x=>x.zone).length
  };
}
function backupPayload(){
  normalizeSaveData();
  return {game:'JardinDex FP Safor', type:'jardindex-save', version:2, exportedAt:new Date().toISOString(), data:state.data};
}
function encodeBackup(){
  const json=JSON.stringify(backupPayload());
  return 'JDX-SAFOR-2:'+btoa(unescape(encodeURIComponent(json)));
}
function decodeBackup(code){
  const raw=String(code||'').trim().replace(/\s+/g,'');
  const clean=raw.startsWith('JDX-SAFOR-2:')?raw.slice('JDX-SAFOR-2:'.length):raw;
  const json=decodeURIComponent(escape(atob(clean)));
  const payload=JSON.parse(json);
  if(!payload || payload.type!=='jardindex-save' || !payload.data || !payload.data.dex || !payload.data.garden){throw new Error('Código no válido para JardinDex FP Safor');}
  return payload;
}
function renderProfile(){
  const st=profileStats();
  const player=state.data.player||{name:'',group:''};
  app.innerHTML=header()+`<div class="card dark"><h2>👤 Perfil JardinDex</h2><p>Personaliza la colección y usa un código largo para mover el progreso entre ordenadores sin crear archivos.</p></div>
  <div class="grid">
    <div class="card span-6 profile-card">
      <h2>Mi JardinDex</h2>
      <label>Nombre del alumno/a</label>
      <input id="playerName" value="${escapeHtml(player.name)}" placeholder="Escribe tu nombre o alias" oninput="updateProfileField('name',this.value)">
      <label>Grupo / clase</label>
      <input id="playerGroup" value="${escapeHtml(player.group)}" placeholder="Ej. 1º GM Jardinería" oninput="updateProfileField('group',this.value)">
      <div class="profile-badge"><span>🌿</span><div><b>${player.name?escapeHtml(player.name):'JardinDex sin nombre'}</b><small>${player.group?escapeHtml(player.group):'Grupo no indicado'}</small></div></div>
      <div class="profile-stats"><div><b>${st.seen}/74</b><small>vistas</small></div><div><b>${st.implanted}</b><small>en mantenimiento</small></div><div><b>${st.gold}</b><small>doradas</small></div><div><b>${st.zoned}</b><small>ubicadas</small></div></div>
      <p><b>Último guardado:</b> ${niceDate(state.data.updatedAt)}</p>
    </div>
    <div class="card span-6 profile-card">
      <h2>Exportar progreso como código</h2>
      <p>Genera un código largo con toda la partida: nombre, JardiDex, zonas, mantenimiento, plantas completadas y diario técnico.</p>
      <button class="btn primary" onclick="generateBackupCode()">📤 Generar código</button>
      <button class="btn" onclick="copyBackupCode()">📋 Copiar código</button>
      <textarea id="backupCode" class="backup-area" placeholder="Aquí aparecerá el código de exportación."></textarea>
    </div>
    <div class="card span-12 profile-card">
      <h2>Importar progreso desde código</h2>
      <p>Pega aquí el código largo de una JardinDex guardada. Al importar, sustituye el progreso actual de este navegador.</p>
      <textarea id="importCode" class="backup-area import" placeholder="Pega aquí el código JDX-SAFOR-2..."></textarea>
      <div class="stats-row"><button class="btn gold" onclick="importBackupCode()">📥 Importar código</button><button class="btn ghost" onclick="document.getElementById('importCode').value=''">Limpiar</button></div>
    </div>
  </div>`;
}
function updateProfileField(field,value){
  state.data.player=state.data.player||{name:'',group:''};
  state.data.player[field]=String(value||'').slice(0,60);
  save();
}
function generateBackupCode(){
  const el=document.getElementById('backupCode');
  if(el){el.value=encodeBackup(); el.focus(); el.select();}
  toast('Código de JardinDex generado.');
}
async function copyBackupCode(){
  const el=document.getElementById('backupCode');
  if(el && !el.value) el.value=encodeBackup();
  try{await navigator.clipboard.writeText(el.value); toast('Código copiado al portapapeles.');}
  catch(e){ if(el){el.focus(); el.select();} toast('Selecciona y copia el código manualmente.'); }
}
function importBackupCode(){
  const el=document.getElementById('importCode');
  try{
    const payload=decodeBackup(el?.value||'');
    const name=payload.data.player?.name||'sin nombre';
    if(!confirm(`Vas a importar la JardinDex de ${name}. Esto sustituirá el progreso actual. ¿Continuar?`)) return;
    state.data=payload.data; normalizeSaveData(); save();
    toast('JardinDex importada correctamente.');
    renderProfile();
  }catch(e){toast('No he podido importar el código. Revisa que esté completo.', 'bad');}
}
...
function resetGame(){ const ok=prompt("Para borrar todo el progreso escribe BORRAR"); if(ok==="BORRAR"){localStorage.removeItem("jardindex_fp_safor_save"); location.reload();}}
