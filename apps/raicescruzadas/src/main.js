...
class BaseScene extends Phaser.Scene{
  ...
  showItemDetail(item){
    ...
    const depth=200;
    const shade=this.add.rectangle(W/2,H/2,W,H,0x000000,.72).setDepth(depth).setInteractive();
    const panel=this.add.rectangle(W/2,H/2,720,470,0x071425,.98).setStrokeStyle(4,albumColor(item.album),.95).setDepth(depth+1);
    const icon=item.icon || (item.slot?slotIcon(item.slot):'✦');
    this.add.circle(W/2-260,H/2-128,54,albumColor(item.album),.95).setDepth(depth+2);
    this.add.text(W/2-260,H/2-128,icon,{fontSize:'42px',fontStyle:'900',color:'#071425'}).setOrigin(.5).setDepth(depth+3);
    this.add.text(W/2-180,H/2-178,item.name,{fontSize:'30px',fontStyle:'900',color:'#ffffff',wordWrap:{width:470}}).setDepth(depth+2);
    this.add.text(W/2-180,H/2-130,`${item.album} · ${item.rarity}${item.slot?' · '+slotLabel(item.slot):''}`,{fontSize:'18px',fontStyle:'800',color:'#ffe66d'}).setDepth(depth+2);
    this.add.text(W/2-300,H/2-48,item.desc||'Objeto de colección de Raíces Cruzadas.',{fontSize:'20px',color:'#dce7f5',wordWrap:{width:600},lineSpacing:6}).setDepth(depth+2);
    this.add.text(W/2-300,H/2+48,item.flavor||'Una pieza más para completar el álbum.',{fontSize:'18px',fontStyle:'800',color:'#48f5a4',wordWrap:{width:600},lineSpacing:6}).setDepth(depth+2);
    this.add.text(W/2-300,H/2+126,owned?`Conseguido: ${itemDate(item.id)}`:'Todavía no conseguido. Puedes obtenerlo en cajas, tienda o recompensas.',{fontSize:'17px',color:owned?'#70d6ff':'#b9c5d6',wordWrap:{width:600}}).setDepth(depth+2);
    this.btn(W/2,H/2+190,190,44,'Cerrar',()=>{shade.destroy(); this.scene.restart(this._lastSceneData||{});},{fill:0x1b263d,stroke:0xffffff,size:'18px'});
  }
  showMatemonDetail(m){
    if(!m)return; const rec=window.SAVE.matemon[m.id]; const owned=!!rec; const depth=200;
    const shade=this.add.rectangle(W/2,H/2,W,H,0x000000,.72).setDepth(depth).setInteractive();
    this.add.rectangle(W/2,H/2,720,470,0x101033,.98).setStrokeStyle(4,0xffd166,.95).setDepth(depth+1);
    if(owned)this.add.image(W/2-245,H/2-60,m.id).setDisplaySize(190,190).setDepth(depth+2); else this.add.text(W/2-245,H/2-60,'?',{fontSize:'130px',fontStyle:'900',color:'#657086'}).setOrigin(.5).setDepth(depth+2);
    this.add.text(W/2-110,H/2-178,owned?m.name:'Matemon no descubierto',{fontSize:'32px',fontStyle:'900',color:'#ffffff'}).setDepth(depth+2);
    this.add.text(W/2-110,H/2-126,owned?`${m.type} · ${m.rarity}`:'??? · ???',{fontSize:'20px',fontStyle:'800',color:'#ffe66d'}).setDepth(depth+2);
    this.add.text(W/2-110,H/2-70,owned?m.clue:'Aparecerá en el Matemonario cuando lo obtengas en una Caja Matemon.',{fontSize:'20px',color:'#dce7f5',wordWrap:{width:380},lineSpacing:6}).setDepth(depth+2);
    this.add.text(W/2-110,H/2+28,owned?`Conseguido: ${rec.date||'antes de esta versión'}\nFragmentos: ${rec.fragments||0}`:'Primera copia: desbloquea la carta. Repetidos: dan fragmentos.',{fontSize:'18px',color:owned?'#48f5a4':'#b9c5d6',wordWrap:{width:390},lineSpacing:6}).setDepth(depth+2);
    this.btn(W/2,H/2+190,190,44,'Cerrar',()=>{shade.destroy(); this.scene.restart(this._lastSceneData||{});},{fill:0x1b263d,stroke:0xffffff,size:'18px'});
  }
}
