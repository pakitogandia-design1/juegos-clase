...
function mixColor(a,b,t){ const ca=Phaser.Display.Color.IntegerToRGB(a), cb=Phaser.Display.Color.IntegerToRGB(b); const m=(x,y)=>Math.max(0,Math.min(255,Math.round(x+(y-x)*t))); return Phaser.Display.Color.GetColor(m(ca.r,cb.r),m(ca.g,cb.g),m(ca.b,cb.b)); }
function lighten(color,amt=.2){ return mixColor(color,0xffffff,amt); }
function darken(color,amt=.2){ return mixColor(color,0x000000,amt); }
...
class BaseScene extends Phaser.Scene{
  ...
  makePopupCloseButton(x,y,label,close,depth){
    const bg=this.add.rectangle(x,y,150,42,0x1b263d,.96).setStrokeStyle(2,0xffffff,.75).setDepth(depth).setInteractive({useHandCursor:true});
    const tx=this.add.text(x,y,label,{fontSize:'17px',fontStyle:'900',color:'#ffffff'}).setOrigin(.5).setDepth(depth+1);
    bg.on('pointerover',()=>bg.setFillStyle(0x2b3d63,.98));
    bg.on('pointerout',()=>bg.setFillStyle(0x1b263d,.96));
  ...
  showItemDetail(item){
    ...
    const close=()=>{while(objs.length){const o=objs.pop(); if(o&&o.destroy)o.destroy();}};
    const shade=add(this.add.rectangle(W/2,H/2,W,H,0x000000,.72).setDepth(depth).setInteractive());
    shade.on('pointerdown', close);
    add(this.add.rectangle(W/2,H/2,720,470,0x071425,.98).setStrokeStyle(4,albumColor(item.album),.95).setDepth(depth+1));
    const icon=item.icon || (item.slot?slotIcon(item.slot):'✦');
    add(this.add.circle(W/2-260,H/2-128,54,albumColor(item.album),.95).setDepth(depth+2));
    add(this.add.text(W/2-260,H/2-128,icon,{fontSize:'42px',fontStyle:'900',color:'#071425'}).setOrigin(.5).setDepth(depth+3));
    add(this.add.text(W/2-180,H/2-178,item.name,{fontSize:'30px',fontStyle:'900',color:'#ffffff',wordWrap:{width:470}}).setDepth(depth+2));
    add(this.add.text(W/2-180,H/2-130,`${item.album} · ${item.rarity}${item.slot?' · '+slotLabel(item.slot):''}`,{fontSize:'18px',fontStyle:'800',color:'#ffe66d'}).setDepth(depth+2));
    add(this.add.text(W/2-300,H/2-48,item.desc||'Objeto de colección de Raíces Cruzadas.',{fontSize:'20px',color:'#dce7f5',wordWrap:{width:600},lineSpacing:6}).setDepth(depth+2));
    add(this.add.text(W/2-300,H/2+48,item.flavor||'Una pieza más para completar el álbum.',{fontSize:'18px',fontStyle:'800',color:'#48f5a4',wordWrap:{width:600},lineSpacing:6}).setDepth(depth+2));
    add(this.add.text(W/2-300,H/2+126,owned?`Conseguido: ${itemDate(item.id)}`:'Todavía no conseguido. Puedes obtenerlo en cajas, tienda o recompensas.',{fontSize:'17px',color:owned?'#70d6ff':'#b9c5d6',wordWrap:{width:600}}).setDepth(depth+2));
    this.makePopupCloseButton(W/2+268,H/2-194,'← Atrás',close,depth+4).forEach(add);
    this.makePopupCloseButton(W/2,H/2+190,'Cerrar',close,depth+4).forEach(add);
  }
  showMatemonDetail(m){
    if(!m)return; const rec=window.SAVE.matemon[m.id]; const owned=!!rec; const depth=200;
    const objs=[];
    const add=(o)=>{objs.push(o); return o;};
    const close=()=>{while(objs.length){const o=objs.pop(); if(o&&o.destroy)o.destroy();}};
    const shade=add(this.add.rectangle(W/2,H/2,W,H,0x000000,.72).setDepth(depth).setInteractive());
    shade.on('pointerdown', close);
    add(this.add.rectangle(W/2,H/2,720,470,0x101033,.98).setStrokeStyle(4,0xffd166,.95).setDepth(depth+1));
    if(owned)add(this.add.image(W/2-245,H/2-60,m.id).setDisplaySize(190,190).setDepth(depth+2)); else add(this.add.text(W/2-245,H/2-60,'?',{fontSize:'130px',fontStyle:'900',color:'#657086'}).setOrigin(.5).setDepth(depth+2));
    add(this.add.text(W/2-110,H/2-178,owned?m.name:'Matemon no descubierto',{fontSize:'32px',fontStyle:'900',color:'#ffffff'}).setDepth(depth+2));
    add(this.add.text(W/2-110,H/2-126,owned?`${m.type} · ${m.rarity}`:'??? · ???',{fontSize:'20px',fontStyle:'800',color:'#ffe66d'}).setDepth(depth+2));
    add(this.add.text(W/2-110,H/2-70,owned?m.clue:'Aparecerá en el Matemonario cuando lo obtengas en una Caja Matemon.',{fontSize:'20px',color:'#dce7f5',wordWrap:{width:380},lineSpacing:6}).setDepth(depth+2));
    add(this.add.text(W/2-110,H/2+28,owned?`Conseguido: ${rec.date||'antes de esta versión'}\nFragmentos: ${rec.fragments||0}`:'Primera copia: desbloquea la carta. Repetidos: dan fragmentos.',{fontSize:'18px',color:owned?'#48f5a4':'#b9c5d6',wordWrap:{width:390},lineSpacing:6}).setDepth(depth+2));
    this.makePopupCloseButton(W/2+268,H/2-194,'← Atrás',close,depth+4).forEach(add);
    this.makePopupCloseButton(W/2,H/2+190,'Cerrar',close,depth+4).forEach(add);
  }
}
...
class PlayScene extends BaseScene{constructor(){super('PlayScene')}init(data){this.cfg=data||{};}create(){const world=this.cfg.world||'Multiverso';this.theme=this.addBg(world);this.title('Crucigrama','Selecciona una palabra, lee la definición y escribe la respuesta completa');this.topBar();this.back('ModeScene');
 ...
 finish(){window.SAVE.stats.played++; window.SAVE.resources.seeds+=80; window.SAVE.resources.keys+=1; const ach=unlockAchievement('first_cross','Primer crucigrama completado',{seeds:100,keys:1}); const rewards=[`+80 semillas`, '+1 llave de vivero']; if(this.mistakes===0){window.SAVE.stats.perfect++; window.SAVE.resources.keys+=1; rewards.push('+1 llave extra por perfecto'); unlockAchievement('first_perfect','Primer perfecto',{seeds:150,keys:1});} const r=rewardItem(this.cfg.world); const rewardObjs=rewards.map(t=>({kind:'recurso',text:t,title:t})); rewardObjs.push(r); save(); this.scene.start('RewardScene',{title:'Crucigrama completado',rewards:rewardObjs,from:'ModeScene'});}
}
class RewardScene extends BaseScene{constructor(){super('RewardScene')}init(data){this.dataIn=data||{};}create(){this._lastSceneData=this.dataIn; this.addBg('Multiverso');this.title(this.dataIn.title||'Recompensas','Haz clic en una recompensa para ver más información');this.topBar(); const panel=this.add.rectangle(W/2,H/2,900,500,0x071425,.92).setStrokeStyle(3,0x42e8ff,.7); const rewards=this.dataIn.rewards||[]; if(!rewards.length){(this.dataIn.lines||[]).forEach((l)=>rewards.push({kind:'text',text:l,title:l}));}
  rewards.forEach((r,i)=>{const x=250+(i%3)*300,y=245+Math.floor(i/3)*145; const item=r.item; const matemon=r.matemon; const col=item?albumColor(item.album):(matemon?0xffd166:(r.kind==='dust'||r.kind==='fragments'?0xffe66d:0x48f5a4)); const card=this.add.rectangle(x,y,245,118,0x101c31,.96).setStrokeStyle(3,col,.9).setInteractive({useHandCursor:!!(item||matemon)}); this.add.circle(x-92,y-28,30,col,.95); this.add.text(x-92,y-28,item?(item.icon||slotIcon(item.slot)):matemon?'✨':r.kind==='dust'?'✨':r.kind==='fragments'?'🧩':'🌱',{fontSize:'27px',fontStyle:'900',color:'#071425'}).setOrigin(.5); const title=item?item.name:(matemon?matemon.name:(r.title||r.text)); this.add.text(x-52,y-52,title,{fontSize:'17px',fontStyle:'900',color:'#ffffff',wordWrap:{width:150}}); this.add.text(x-52,y-17,item?`${item.album} · ${item.rarity}`:matemon?`${matemon.type} · ${matemon.rarity}`:(r.kind||'recurso'),{fontSize:'12px',fontStyle:'800',color:'#ffe66d'}); this.add.text(x-108,y+18,r.text||'',{fontSize:'12px',color:'#c8d5e8',wordWrap:{width:215}}); if(item)card.on('pointerdown',()=>this.showItemDetail(item)); if(matemon)card.on('pointerdown',()=>this.showMatemonDetail(matemon)); });
  this.btn(W/2-180,620,260,58,'Abrir más cajas',()=>this.scene.start('BoxesScene')); this.btn(W/2+180,620,260,58,'Volver',()=>this.scene.start(this.dataIn.from||'MenuScene'));}
}
class BoxesScene extends BaseScene{constructor(){super('BoxesScene')}create(){this.addBg('Multiverso');this.title('Cajas animadas','Abre cajas y haz clic en las recompensas para ver su ficha');this.topBar();this.back(); const boxes=[['Básica','seeds',80,'Multiverso'],['Verde','keys',1,'Mundo Verde'],['LibrerIA','keys',1,'LibrerIA'],['Series','keys',1,'Series'],['Sagas','keys',1,'Sagas'],['Videojuegos','keys',1,'Videojuegos'],['Brainrot','keys',1,'Brainrot'],['Matemon','keys',1,'Matemon'],['Multiverso','keys',2,'Multiverso'],['Prohibida','forbidden',1,'Prohibido']]; boxes.forEach((b,i)=>this.btn(150+(i%5)*245,165+Math.floor(i/5)*106,210,72,`${b[0]}
${b[2]} ${b[1]==='seeds'?'semillas':b[1]==='keys'?'llave(s)':'prohibida'}`,()=>this.openBox(b),{fill:0x111b31,stroke:i===9?0xff4fd8:0xffd166,size:'15px'})); this.add.text(80,610,'Las recompensas aparecen como cartas pequeñas. Puedes clicar cada objeto o Matemon para abrir su ficha detallada.',{fontSize:'16px',color:'#b9c5d6'});}
 openBox(b){const [name,res,cost,album]=b; if(window.SAVE.resources[res]<cost){this.toast('No tienes recursos suficientes. Puedes convertir en Tienda.');return;} window.SAVE.resources[res]-=cost; window.SAVE.stats.boxes++; const cx=640,cy=410; const box=this.add.container(cx,cy); const base=this.add.rectangle(0,20,170,110,0x8a4f1d,1).setStrokeStyle(4,0xffd166); const lid=this.add.rectangle(0,-45,190,52,0xb86b28,1).setStrokeStyle(4,0xffe66d); const glow=this.add.circle(0,0,12,0xffe66d,.0); box.add([glow,base,lid]); this.tweens.add({targets:box,x:'+=8',yoyo:true,repeat:8,duration:50,onComplete:()=>{this.tweens.add({targets:lid,y:-130,angle:-18,duration:380,ease:'Back.Out'}); this.tweens.add({targets:glow,scale:28,alpha:.5,duration:420,yoyo:true,onComplete:()=>{const rewards=[]; const pulls=name==='Prohibida'?5:name==='Multiverso'?4:3; for(let i=0;i<pulls;i++){ if(album==='Matemon'&&i===0){const m=pick(RC_DATA.matemon); const id=m.id; if(window.SAVE.matemon[id]){window.SAVE.resources.fragments+=2; if(typeof window.SAVE.matemon[id]==='object') window.SAVE.matemon[id].fragments=(window.SAVE.matemon[id].fragments||0)+2; rewards.push({kind:'fragments',text:`Repetido: +2 fragmentos de ${m.name}`,matemon:m});}else{window.SAVE.matemon[id]={owned:true,date:new Date().toLocaleDateString('es-ES'),fragments:0}; rewards.push({kind:'matemon',text:`Matemon nuevo: ${m.name}`,matemon:m});} } else {rewards.push(rewardItem(album));} } save(); this.scene.start('RewardScene',{title:`Caja ${name} abierta`,rewards,from:'BoxesScene'});}});}});}
}
class WardrobeScene extends BaseScene{
  ...
  drawAvatar(){
    ...
    const outfit=RC_DATA.items.find(i=>i.id===slots.outfit), hat=RC_DATA.items.find(i=>i.id===slots.hat), acc=RC_DATA.items.find(i=>i.id===slots.accessory), badge=RC_DATA.items.find(i=>i.id===slots.badge), frame=RC_DATA.items.find(i=>i.id===slots.frame), bgItem=RC_DATA.items.find(i=>i.id===slots.background);
    this.add.rectangle(300,360,372,530,0x000000,.28).setStrokeStyle(2,0xffffff,.14);
    const c=this.add.container(300,352);
    this.drawAvatarBackdrop(c,bgItem);
    this.drawBaseFigure(c);
    if(outfit) this.drawOutfitGraphic(c,outfit);
    if(acc) this.drawAccessoryGraphic(c,acc);
    if(badge) this.drawBadgeGraphic(c,badge);
    if(hat) this.drawHatGraphic(c,hat);
    if(frame) this.drawFrameGraphic(c,frame);
    const equipped=[hat&&`Gorro: ${hat.name}`,outfit&&`Ropa: ${outfit.name}`,acc&&`Accesorio: ${acc.name}`,badge&&`Insignia: ${badge.name}`,frame&&`Marco: ${frame.name}`,bgItem&&`Fondo: ${bgItem.name}`].filter(Boolean);
    this.add.text(120,575,equipped.length?equipped.join(' · '):'Avatar básico sin equipamiento',{fontSize:'14px',color:'#d9e4f4',wordWrap:{width:360},align:'center'}).setOrigin(0,0.5);
    this.add.text(172,612,'Las prendas se dibujan con formas y capas: gorro arriba, ropa sobre el cuerpo, accesorio lateral e insignia en el pecho.',{fontSize:'14px',color:'#c8d5e8',wordWrap:{width:280},align:'center'});
  }
  drawAvatarBackdrop(c,bgItem){
    const base=bgItem?albumColor(bgItem.album):0x173420; const glow=lighten(base,.28); const deep=darken(base,.28);
    c.add(this.add.circle(0,0,154,base,.68));
    c.add(this.add.circle(0,0,126,glow,.18));
    const g=this.add.graphics();
    g.lineStyle(3,glow,.34);
    for(let i=0;i<6;i++){ const r=78+i*12; g.strokeCircle(0,0,r); }
    g.lineStyle(2,deep,.25);
    for(let i=0;i<10;i++){ const a=(Math.PI*2/10)*i; g.beginPath(); g.moveTo(Math.cos(a)*18,Math.sin(a)*18); g.lineTo(Math.cos(a)*140,Math.sin(a)*140); g.strokePath(); }
    c.add(g);
    if(bgItem){
      c.add(this.add.text(0,-3,bgItem.icon||'✦',{fontSize:'72px',fontStyle:'900',color:'#ffffff',alpha:.10}).setOrigin(.5));
    }
  }
  drawBaseFigure(c){
    const skin=0xffd3a3, hair=0x2f241e, outline=0x071425;
    const g=this.add.graphics();
    g.fillStyle(0x000000,.20); g.fillEllipse(0,196,150,28);
    g.fillStyle(0x2a324a,1); g.fillRoundedRect(-48,126,32,84,12); g.fillRoundedRect(16,126,32,84,12); // legs
    g.fillStyle(0x111827,1); g.fillRoundedRect(-56,196,46,18,8); g.fillRoundedRect(10,196,46,18,8); // shoes
    g.fillStyle(0xd7e7ef,1); g.fillRoundedRect(-54,-2,108,150,26); // torso base
    g.fillStyle(skin,1); g.fillRoundedRect(-76,18,24,102,12); g.fillRoundedRect(52,18,24,102,12); // arms
    g.fillCircle(-64,124,12); g.fillCircle(64,124,12); // hands
    g.fillStyle(skin,1); g.fillCircle(0,-92,54); // head
    g.fillStyle(hair,1); g.fillEllipse(0,-118,110,54); g.fillRoundedRect(-55,-120,110,32,18); // hair cap
    g.lineStyle(3,outline,.85); g.strokeCircle(0,-92,54);
    g.fillStyle(0xffffff,1); g.fillEllipse(-18,-99,15,10); g.fillEllipse(18,-99,15,10); g.fillStyle(outline,1); g.fillCircle(-18,-98,4); g.fillCircle(18,-98,4);
    g.lineStyle(3,outline,.85); g.beginPath(); g.arc(0,-74,18,0.15,2.95,false); g.strokePath();
    g.lineStyle(3,outline,.25); g.strokeRoundedRect(-54,-2,108,150,26);
    c.add(g);
  }
  drawOutfitGraphic(c,item){
    const base=albumColor(item.album), trim=lighten(base,.35), shade=darken(base,.28); const name=(item.name||'').toLowerCase();
    const g=this.add.graphics();
    if(/mono|buzo|peto|jardinero/.test(name)){
      g.fillStyle(base,1); g.fillRoundedRect(-50,-12,100,166,20);
      g.fillStyle(trim,.95); g.fillRoundedRect(-30,-26,16,70,6); g.fillRoundedRect(14,-26,16,70,6);
      g.fillStyle(shade,1); g.fillRoundedRect(-28,44,56,40,8);
      g.fillStyle(0xffd166,1); g.fillCircle(-22,-2,4); g.fillCircle(22,-2,4);
    } else if(/capa|tunica|túnica|fantasma/.test(name)){
      g.fillStyle(base,.92); g.fillRoundedRect(-56,-16,112,154,26);
      g.fillTriangle(-56,-8,-94,78,-58,122); g.fillTriangle(56,-8,94,78,58,122);
      g.fillStyle(trim,.22); g.fillRoundedRect(-42,0,84,118,18);
    } else if(/armadura|dragon|dragón/.test(name)){
      g.fillStyle(base,1); g.fillRoundedRect(-56,-10,112,150,20);
      g.fillStyle(trim,1); g.fillRoundedRect(-42,2,84,24,10); g.fillRoundedRect(-38,38,76,26,10); g.fillRoundedRect(-34,76,68,24,10);
      g.fillStyle(shade,1); g.fillTriangle(-56,10,-92,54,-56,88); g.fillTriangle(56,10,92,54,56,88);
    } else if(/abeja/.test(name)){
      for(let i=0;i<7;i++){ g.fillStyle(i%2?0x1d1b16:base,1); g.fillRoundedRect(-52,-10+i*22,104,14,6); }
      g.fillStyle(0xffffff,.28); g.fillEllipse(-64,34,42,80); g.fillEllipse(64,34,42,80);
    } else {
      g.fillStyle(base,1); g.fillRoundedRect(-54,-12,108,154,24);
      g.fillStyle(trim,.18); g.fillRoundedRect(-42,2,84,120,20);
      g.fillStyle(trim,.95); g.fillRoundedRect(-30,-16,18,56,6); g.fillRoundedRect(12,-16,18,56,6);
      g.fillStyle(shade,.95); g.fillRoundedRect(-36,54,72,34,10);
    }
    g.lineStyle(3,0xffffff,.45); g.strokeRoundedRect(-54,-12,108,154,24);
    c.add(g);
    c.add(this.add.text(0,66,item.icon||slotIcon(item.slot),{fontSize:'28px',fontStyle:'900',color:'#ffffff',alpha:.9}).setOrigin(.5));
  }
  drawHatGraphic(c,item){
    const base=albumColor(item.album), trim=lighten(base,.35), deep=darken(base,.25); const name=(item.name||'').toLowerCase();
    const g=this.add.graphics();
    if(/corona/.test(name)){
      g.fillStyle(base,1); g.fillTriangle(-54,-150,-28,-198,-8,-150); g.fillTriangle(-8,-150,0,-206,8,-150); g.fillTriangle(8,-150,28,-196,54,-150); g.fillRoundedRect(-58,-150,116,20,8);
    } else if(/casco|helmet/.test(name)){
      g.fillStyle(base,1); g.fillEllipse(0,-152,128,70); g.fillStyle(deep,.9); g.fillRoundedRect(-62,-150,124,14,5); g.fillStyle(trim,.9); g.fillRoundedRect(-12,-180,24,20,6);
    } else if(/sombrero|hat/.test(name)){
      g.fillStyle(base,1); g.fillEllipse(0,-144,144,24); g.fillRoundedRect(-40,-188,80,48,12); g.fillStyle(trim,.95); g.fillRoundedRect(-36,-154,72,10,4);
    } else if(/gorra|cap/.test(name)){
      g.fillStyle(base,1); g.fillEllipse(-4,-150,120,42); g.fillRoundedRect(-40,-172,80,24,10); g.fillStyle(trim,.95); g.fillTriangle(2,-148,62,-136,14,-130);
    } else {
      g.fillStyle(base,1); g.fillRoundedRect(-42,-182,84,46,16); g.fillEllipse(0,-136,130,22);
    }
    g.lineStyle(3,0xffffff,.55); g.strokePath();
    c.add(g);
    c.add(this.add.text(0,-158,item.icon||slotIcon(item.slot),{fontSize:'22px',fontStyle:'900',color:'#ffffff'}).setOrigin(.5));
  }
  drawAccessoryGraphic(c,item){
    const base=albumColor(item.album), trim=lighten(base,.35), name=(item.name||'').toLowerCase();
    const g=this.add.graphics();
    g.lineStyle(6,0x2a324a,.75); g.beginPath(); g.moveTo(22,12); g.lineTo(92,42); g.strokePath();
    if(/libro|book/.test(name)){
      g.fillStyle(base,1); g.fillRoundedRect(84,22,52,42,8); g.fillStyle(trim,.9); g.fillRect(108,26,4,34);
    } else if(/vara|varita|wand/.test(name)){
      g.lineStyle(8,base,1); g.beginPath(); g.moveTo(84,68); g.lineTo(134,18); g.strokePath(); g.fillStyle(trim,1); g.fillStar(140,12,5,8,15);
    } else if(/mochila|bag/.test(name)){
      g.fillStyle(base,1); g.fillRoundedRect(80,20,58,68,12); g.fillStyle(trim,.9); g.fillRoundedRect(92,30,34,18,6);
    } else if(/espada|sword/.test(name)){
      g.fillStyle(trim,1); g.fillTriangle(96,14,130,64,114,72); g.fillStyle(base,1); g.fillRoundedRect(78,64,42,10,4);
    } else {
      g.fillStyle(base,1); g.fillCircle(110,44,28); g.fillStyle(trim,.9); g.fillCircle(110,44,14);
    }
    g.lineStyle(3,0xffffff,.45); g.strokePath();
    c.add(g);
    c.add(this.add.text(110,44,item.icon||slotIcon(item.slot),{fontSize:'18px',fontStyle:'900',color:'#ffffff'}).setOrigin(.5));
  }
  drawBadgeGraphic(c,item){
    const base=albumColor(item.album), trim=lighten(base,.28);
    const g=this.add.graphics();
    g.fillStyle(base,1); g.fillStar(-46,24,6,14,26); g.lineStyle(3,trim,.9); g.strokePath();
    c.add(g);
    c.add(this.add.text(-46,24,item.icon||slotIcon(item.slot),{fontSize:'14px',fontStyle:'900',color:'#ffffff'}).setOrigin(.5));
  }
  drawFrameGraphic(c,item){
    const base=albumColor(item.album), trim=lighten(base,.35), deep=darken(base,.25);
    const g=this.add.graphics();
    g.lineStyle(8,base,.92); g.strokeRoundedRect(-144,-222,288,438,26);
    g.lineStyle(3,trim,.95); g.strokeRoundedRect(-132,-210,264,414,18);
    g.fillStyle(base,.95); [[-128,-206],[128,-206],[-128,206],[128,206]].forEach(([x,y])=>{g.fillCircle(x,y,10); g.fillTriangle(x,y-18,x+18,y,x,y+18);});
    c.add(g);
    c.add(this.add.text(0,184,item.icon||slotIcon(item.slot),{fontSize:'18px',fontStyle:'900',color:Phaser.Display.Color.IntegerToColor(deep).rgba}).setOrigin(.5));
  }
...
class CollectionScene extends BaseScene{
  ...
  create(){
    this._lastSceneData={album:this.album,page:this.page};
    this.addBg(this.album||'Multiverso'); this.title(this.album?`Álbum ${this.album}`:'Colección visual','Fichas clicables con descripción, rareza, ranura y fecha de consecución'); this.topBar(); this.back();
  ...
  drawAlbums(){
    ...
    this.add.text(80,648,`Total de colección: ${Object.keys(window.SAVE.items).length}/${RC_DATA.items.length}. Hay muchos más objetos temáticos, equipables y piezas raras para que las cajas siempre tengan gracia.`,{fontSize:'16px',color:'#b9c5d6'});
  }
  drawAlbumCards(album){
    ...
    slice.forEach((it,i)=>{const x=165+(i%4)*300,y=230+Math.floor(i/4)*135; const own=ownedItem(it.id), col=own?albumColor(it.album):0x596174; const card=this.add.rectangle(x,y,260,112,own?0x101c31:0x080b15,.94).setStrokeStyle(3,col,.85).setInteractive({useHandCursor:true}); this.add.circle(x-98,y-30,24,col,own?.95:.22); this.add.text(x-98,y-30,own?(it.icon||slotIcon(it.slot)):'?',{fontSize:'23px',fontStyle:'900',color:'#071425'}).setOrigin(.5); this.add.text(x-62,y-48,own?it.name:'???',{fontSize:'18px',fontStyle:'900',color:own?'#ffffff':'#7e8797',wordWrap:{width:188}}); this.add.text(x-62,y-20,`${it.rarity}${it.slot?' · '+slotLabel(it.slot):''}`,{fontSize:'13px',fontStyle:'800',color:own?'#ffe66d':'#8d96a8'}); this.add.text(x-112,y+8,own?it.desc:'Objeto aún no conseguido. Abre cajas, juega crucigramas o visita la tienda.',{fontSize:'12px',color:own?'#c8d5e8':'#778196',wordWrap:{width:224}}); this.add.text(x-112,y+42,own?`Conseguido: ${itemDate(it.id)}`:'Fecha: pendiente',{fontSize:'11px',color:own?'#48f5a4':'#6d7688'}); card.on('pointerdown',()=>{ if(own)this.showItemDetail(it); else this.toast('Todavía no conseguido. Las cajas intentan priorizar objetos nuevos.'); });});
...
class MatemonScene extends BaseScene{
  ...
  create(){
    this._lastSceneData={page:this.page};
...
class OptionsScene extends BaseScene{constructor(){super('OptionsScene')}create(){this.addBg('Extras');this.title('Opciones y diagnóstico','Herramientas para reparar, exportar o importar sin borrar todo');this.topBar();this.back(); const opts=[['Reparar progreso',()=>{ensureSave();repairWardrobe();this.toast('Progreso reparado.')}],['Recalcular / reparar armario',()=>{repairWardrobe();this.toast('Armario validado.')}],['Exportar progreso',()=>this.exportSave()],['Importar progreso',()=>this.importSave()],['Borrar progreso',()=>{localStorage.removeItem(LS_KEY);location.reload();}]]; opts.forEach((o,i)=>this.btn(300+(i%2)*380,180+Math.floor(i/2)*90,320,58,o[0],o[1],{fill:0x111b31,stroke:i===4?0xff6b6b:0x42e8ff,size:'18px'}));}
