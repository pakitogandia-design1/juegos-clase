...
function iconForItem(it){
  ...
  if(it.type==='pet') return it.rarity==='legendary'?'🐉':it.rarity==='epic'?'🤖':'🐾';
...
function drawAvatar(){
  ...
  return `<div class="avatar-card"><div class="avatar"><div class="hat-shape" title="${hat?.name||''}"></div><div class="hair"></div><div class="head"></div><div class="eye l"></div><div class="eye r"></div><div class="smile"></div><div class="arm l"></div><div class="arm r"></div><div class="body"></div></div><div class="pet" title="${pet?.name||''}"><div class="blob"></div><div class="eye l"></div><div class="eye r"></div><div class="mouth"></div></div><div class="profile-label"><h2>Jugador/a ProporcioLab</h2><p>${badge?.name || 'Aprendiz visual'} · Mascota: ${pet?.name || 'Directín'} · Gorro: ${hat?.name || 'Gorra de Aprendiz'}</p></div></div>`;
}
function showMain(){
