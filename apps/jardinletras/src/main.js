/* Jardín de Letras V1.0.10 - Phaser 3 */
(function(){
'use strict';
...
function initialSave(){ return {version:'1.0.10', profile:{name:'Jugador/a',createdAt:today(),equipped:{}}, codex:{}, items:{}, achievements:{}, stats:{games:0,perfect:0,expeditions:0,boxes:0,missionsDone:0,wordsFound:0}, resources:{seeds:120,dust:0,keys:1,secretKeys:0}, activeGame:null, activeExpedition:null, history:[], settings:{tutorialSeen:false, sound:true}}; }
...
class JardinScene extends Phaser.Scene{
 ...
 cleanEquipped(){
  const validSlots=['hat','outfit','badge'];
  const current=this.dataSave.profile.equipped||{};
  const fixed={};
  validSlots.forEach(slot=>{
    let id=current[slot];
    if(Array.isArray(id)) id=id[id.length-1];
    if(id && typeof id==='object') id=id.id;
    const it=allCollection().find(item=>item.id===id && item.slot===slot);
    if(it && this.dataSave.items[it.id]) fixed[slot]=it.id;
  });
  this.dataSave.profile.equipped=fixed;
  return fixed;
 }
 getEquippedItem(slot){ const eq=this.cleanEquipped(); const id=eq[slot]; return id?allCollection().find(i=>i.id===id):null; }
 unequipSlot(slot){ this.cleanEquipped(); delete this.dataSave.profile.equipped[slot]; this.persist(); this.showToast('Objeto desequipado'); this.showWardrobe(); }
 unequipAll(){ this.dataSave.profile.equipped={}; this.persist(); this.showToast('Armario despejado'); this.showWardrobe(); }
 equipItem(it){ this.cleanEquipped(); this.dataSave.profile.equipped[it.slot]=it.id; this.persist(); this.showToast('Objeto equipado'); this.showWardrobe(); }
...
${it.tooltip}`; }
 ...
 drawAvatar(x,y,scale=1){
  const outfit=this.getEquippedItem('outfit');
  const hat=this.getEquippedItem('hat');
  const badge=this.getEquippedItem('badge');
 ...
 this.clear();
 this.cleanEquipped();
 this.decorate('Armario','Equipa y desequipa accesorios sin riesgo: sombreros, indumentaria e insignias.');
 this.smallBack();
 this.panel(55,110,1170,540,COLORS.paper,.94);
 this.drawAvatar(1035,326,1.5);
 const eq=this.dataSave.profile.equipped||{};
 const current={hat:this.getEquippedItem('hat'),outfit:this.getEquippedItem('outfit'),badge:this.getEquippedItem('badge')};
 this.label(90,130,'Equipamiento actual',23,'#2b2116');
 const slots=[['hat','Sombrero',current.hat],['outfit','Ropa',current.outfit],['badge','Insignia',current.badge]];
 slots.forEach((row,i)=>{
   const x=90+i*220,y=165;
   this.panel(x,y,200,92,COLORS.paper2,.98,row[2]?rarityColor(row[2].rarity):COLORS.gray);
   this.label(x+14,y+12,row[1],15,'#4b3a20');
   this.label(x+14,y+36,row[2]?row[2].name:'Vacío',15,'#2b2116','left',170);
   this.button(x+118,y+56,64,26,'Quitar',()=>this.unequipSlot(row[0]),{size:11,color:COLORS.red});
 });
 this.button(760,588,155,42,'Quitar todo',()=>this.unequipAll(),{size:15,color:COLORS.red});
 this.button(935,588,215,42,'Reparar armario',()=>{this.cleanEquipped(); this.persist(); this.showToast('Armario reparado'); this.showWardrobe();},{size:15,color:COLORS.gray});
 const items=allCollection().filter(i=>['Sombreros','Indumentaria','Insignias'].includes(i.category)&&this.dataSave.items[i.id]);
 const per=8,pages=Math.max(1,Math.ceil(items.length/per));
 this.page=clamp(this.page,0,pages-1);
 this.label(90,285,`Accesorios desbloqueados: ${items.length}`,22,'#2b2116');
 this.label(90,313,'Toca una tarjeta para equiparla. Usa los botones superiores para desequipar aunque un objeto visual se solape.',14,'#4b3a20','left',700);
 if(!items.length){ this.label(110,405,'Abre la Caja de Llaves o la Caja Prohibida para conseguir accesorios visibles.',26,'#4b3a20','left',700); }
 items.slice(this.page*per,this.page*per+per).forEach((it,i)=>{
   const x=90+(i%4)*220,y=345+Math.floor(i/4)*120;
   const equipped=this.dataSave.profile.equipped?.[it.slot]===it.id;
   this.panel(x,y,195,96,equipped?0xfff4d8:COLORS.paper2,.98,equipped?COLORS.blue:rarityColor(it.rarity));
   this.drawItemIcon(it,x+36,y+40,.66);
   this.label(x+72,y+14,it.name,14,'#2b2116','left',108);
   this.label(x+72,y+50,`${it.category}
${equipped?'Equipado':'Tocar para equipar'}`,12,'#4b3a20','left',110);
   const z=this.add.zone(x,y,195,96).setOrigin(0).setInteractive({useHandCursor:true});
   z.on('pointerup',()=>{ if(equipped){ this.unequipSlot(it.slot); } else { this.equipItem(it); } });
   this.addObj(z);
 });
