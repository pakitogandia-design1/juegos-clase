...
export class GameScene extends Phaser.Scene {
  ...
  nextTurn(){
    ...
    if(this.shots>0 && this.time.now>this.dropStartAt && this.shots%this.level.dropEveryShots===0) this.dropRows(12);
    this.updateHud();
  }
  ...
  checkDanger(){
    ...
    if(this.grid.all().some(b=>b.y>this.dangerY-18)){
      this.lives--; this.audio.lose(); this.perfect=false; this.dropRows(-60); this.dropGraceMs=30000;
    this.dropStartAt=this.time.now+this.dropGraceMs;
    this.lastDrop=this.dropStartAt;
  ...
  updateHud(tip){
    const payload={mode:this.modeCfg.name,level:this.levelId,score:this.score,streak:this.streak,lives:this.lives,next:this.modeCfg.showNext?this.next?.label:'?', tip:tip||`${this.current?.label} = ? Busca dónde formar un trío. El tablero empieza a bajar tras 30 s.`};
    this.onHud?.(payload);
  }
  usePowerup(type){
