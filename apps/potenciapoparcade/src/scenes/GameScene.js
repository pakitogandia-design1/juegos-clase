...
export class GameScene extends Phaser.Scene {
  ...
  update(time,delta){
    ...
    if(time-this.lastDrop>dropMs && time>this.freezeUntil){ this.dropRows(19); this.lastDrop=time; }
    this.checkDanger();
  }
  drawAim(ang){
  ...
  popGroup(group){
    ...
    this.time.delayedCall(270,()=>{
      ...
      if(this.grid.all().length===0 || this.cleared>=this.level.targetClears) this.levelWin();
    });
  }
  ...
  nextTurn(){
    ...
    if(this.shots>0 && this.shots%this.level.dropEveryShots===0) this.dropRows(24);
    this.updateHud();
  }
  ...
  checkDanger(){
    ...
    if(this.grid.all().some(b=>b.y>this.dangerY-18)){
      this.lives--; this.audio.lose(); this.perfect=false; this.dropRows(-60); this.lastDrop=this.time.now;
  ...
  updateHud(tip){
    const payload={mode:this.modeCfg.name,level:this.levelId,score:this.score,streak:this.streak,lives:this.lives,next:this.modeCfg.showNext?this.next?.label:'?', tip:tip||`${this.current?.label} = ? Busca dónde formar un trío.`};
    this.onHud?.(payload);
  }
  usePowerup(type){
    ...
    if(type==='bomb'){
      const b=Phaser.Utils.Array.GetRandom(this.grid.all()); if(b){ this.audio.pop(); this.grid.removeBubble(b); this.cleared++; this.score+=160; this.updateHud('💥 Bomba raíz: una burbuja menos.'); }
    }
