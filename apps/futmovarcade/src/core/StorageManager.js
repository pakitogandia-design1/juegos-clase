...
export class StorageManager{
 ...
 static migrate(save){save.profile={playerName:'Fran',...(save.profile||{})}; save.settings={music:true,sfx:true,musicVolume:.45,sfxVolume:.75,...(save.settings||{})}; save.stats={...baseStats(),...(save.stats||{})}; save.achievements=save.achievements||{}; save.collection=save.collection||{}; save.records=save.records||{recent:[]}; save.titles=save.titles||['Sin título']; AI_RIVALS.forEach(r=>{save.collection[r.id]={state:'locked',wins:0,losses:0,bestScore:null,firstDefeatMode:null,badges:[],...(save.collection[r.id]||{})}}); return save;}
 static save(data){localStorage.setItem(KEY,JSON.stringify(data));}
 static getPlayerName(save){return (save?.profile?.playerName||'Fran').trim()||'Fran';}
 static setPlayerName(save,name){const clean=(name||'Fran').trim().slice(0,18)||'Fran'; save.profile={...(save.profile||{}),playerName:clean}; return clean;}
